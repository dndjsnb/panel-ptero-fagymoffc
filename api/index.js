...
const penaltyTimes = [30 * 1000, 60 * 1000, 3 * 60 * 1000, 9 * 60 * 1000, 30 * 60 * 1000, 60 * 60 * 1000];
...
app.post('/api/register', async (req, res) => {
    try {
        ...
        const newReseller = new Reseller({ name, email, whatsapp, password, telegram: 'Belum Terhubung', status: 'pending', saldo: 0, ipAddress: clientIp, hardwareId: hardwareId });
...
app.post('/api/request-otp', async (req, res) => {
    try {
        ...
        if (user.telegram === 'Belum Terhubung') return res.status(400).json({ message: "Gagal! Hubungin akun ini ke Bot Telegram dulu, bro." });
        ...
        res.status(500).json({ message: "Server error pas request OTP!" }); 
    }
});
...
app.post('/api/verify-otp', async (req, res) => {
    try {
        ...
        if (!user.otp_expires || currentTime > user.otp_expires) return res.status(400).json({ message: "Kode OTP udah kedaluwarsa. Silakan request ulang." });
        ...
        if (teleIdMatch && teleIdMatch[1]) {
            const VERIF_TOKEN = process.env.VERIF_BOT_TOKEN; 
            await axios.post(`https://api.telegram.org/bot${VERIF_TOKEN}/sendMessage`, { chat_id: teleIdMatch[1], text: `✅ *DATA BERHASIL DIUBAH!*\n\nData akun lu udah sukses di-update lewat website.`, parse_mode: 'Markdown' });
        }
